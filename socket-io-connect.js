              ...
              if(room != null){
                ...
                if(room.number_of_player >= 4){
                  ...
                  if(room.number_of_player == 4){
                    ...
                    //ルームに4人揃ったらゲームを始める
                    initDeck(room.room_name);
                    //players_infoのplayer_nameがroom.current_playerと一致するplayerを探し、その_idを取得する
                    let player_id = room.players_info.find((player) => {
                      return player.player_name == room.current_player;
                    })._id;
                    io.to(room.room_name).emit(SocketConst.EMIT.FIRST_PLAYER, {first_player: player_id, first_card : room.current_field, play_order : 1});
                  }
                }
              }else{
  ...
  initDeck = (room_name) =>{
    Room.findOne({room_name: room_name}, (error, room) => {
      ...
      if(room != null){
        ...
        room.deck = deck;
        //ランダムにcurrent_playerを0~3の間で決める
        room.current_player = Math.floor(Math.random() * 4);
